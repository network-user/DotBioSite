...
## Аналитика (Umami, опционально)

Анонимная self-hosted аналитика (онлайн, посещения, источники, события кликов) с
дашбордом на поддомене `analytics.<домен>`. Поднимается отдельным compose-стеком
в той же сети `dotsound`, трекер проксируется first-party через Caddy портфолио
(`/stats/*`). Ставится одной командой:

```bash
sudo bash deploy/analytics/setup.sh analytics.example.com
```

Полная инструкция (поддомен во фронт-Caddy, `PUBLIC_UMAMI_WEBSITE_ID`,
приватность) - в [analytics/README.md](analytics/README.md).
