...
В репозитории DotSound, в `Caddyfile` (рядом с блоками портфолио и DotLearn -
секция "Co-hosted sites"), добавь site-блок и задай домен через переменную:
...
Домен - в `.env` DotSound (`/opt/dotsound/DotSoundBackend/.env`), голым
хостнеймом без `https://`:

```
ANALYTICS_DOMAIN=analytics.example.com
```

Site-блок обязан быть закоммичен в origin/main DotSound: `deploy.sh` там делает
`git reset --hard`, незакоммиченный блок сотрётся и поддомен пропадёт. Домены в
git не попадают - они в `.env`, который не версионируется.
